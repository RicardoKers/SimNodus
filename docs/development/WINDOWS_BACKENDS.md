...
E-01 may now test a real RC circuit and lifecycle behavior. E-02 may prepare owned firmware and an offline C8 platform, but its external-control acceptance requires SN-019. Qt is unnecessary for both experiments. No upstream SVD/model URL should be downloaded automatically when opening a user project.
