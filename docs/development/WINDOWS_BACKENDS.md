...
E-01 passed its bounded RC/lifecycle profile; see [results](../experiments/E-01-results.md). SN-019 passed Windows control/time, and [E-02](../experiments/E-02-results.md) now boots owned firmware in an offline C8 profile and exercises bounded GPIO/EXTI. Qt is unnecessary. No upstream SVD/model/tool URL is downloaded automatically when opening a user project.

E-02's local build uses the existing CubeIDE Arm GCC without copying or altering the installation. Its GitHub workflow explicitly downloads an exact Arm GNU archive and verifies the vendor-published SHA-256 before extraction. This compiler is a development input, not an application dependency; no C library or compiler runtime is linked into the freestanding ELF. Do not redistribute either toolchain by assumption.
