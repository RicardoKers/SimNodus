...
def main():
    ...
    report = {"status": "passed", "joint_pause_supported": False, "runs": [], "sha256": {
        p.name: e.sha(p) for p in (Path(__file__), e.HERE / "cancel_notify_bridge.cs", e.HERE / "renode-cancellation.patch", e.HERE / "renode-debug-notification.patch",
    ...
    for repetition in range(opts.repetitions):
        ...
        def notify(expected, label):
            path = (directory / (label + ".notification")).resolve()
            send("emulation NotifyCancellationProbe " + str(expected) + " @" + path.as_posix())
            deadline = time.monotonic() + 3
            while not path.exists() and time.monotonic() < deadline:
                time.sleep(0.01)
            e.check(path.exists(), "Notification acknowledgement deadline expired")
        ...
        def outcome(path):
            ...
            return values
        try:
            send("include @" + (e.HERE / "cancel_notify_bridge.cs").as_posix())
            ...
            for step, expected in enumerate((2010875, 2011375)):
                ...
                result.setdefault("premature_notifications", []).append(notify(expected, "premature-" + str(step)))
                e.check(result["premature_notifications"][-1].startswith("rejected:"), "Notification before cancellation accepted")
            ...
            before_notifications = sum(line.startswith("*stopped") for line in mi.lines)
            result["wrong_time_notification"] = notify(running["end"] + 1, "wrong-time")
            e.check(result["wrong_time_notification"].startswith("rejected:"), "Wrong time emitted a stop")
            result["notification"] = notify(running["end"], "valid")
            e.check(result["notification"] == "notified", "Verified pause notification failed")
            line = mi.stop(timeout=2)
            paused = e.stop_record(mi, line, "signal-received")
            e.check('signal-name="SIGINT"' in line and paused["time_ns"] == running["end"], "Wrong debugger stop")
            result["debugger_pause"] = paused
            result["paused_inspection"] = e.inspect_stopped(mi)
            e.check(result["paused_inspection"]["after_ns"] == running["end"], "Paused inspection advanced time")
            result["duplicate_notification"] = notify(running["end"], "duplicate")
            e.check(result["duplicate_notification"].startswith("rejected:"), "Duplicate notification accepted")
            result["paused_thread_info"] = "\n".join(mi.command("52-thread-info", "52^done"))
            e.check('state="stopped"' in result["paused_thread_info"], "GDB still running after notification")
            e.check(sum(line.startswith("*stopped") for line in mi.lines) == before_notifications + 1, "Duplicate stop event")
            path = (directory / "endpoint.txt").resolve()
            start(path, "42", duration=1000, cancel_after=1000)
