...
void execute(int argc, wchar_t** argv)
{
    ...
    Sample final{};
    if(targetNanoseconds > 0)
    {
        require(breakpoint(target), "ngspice rejected the final integration boundary");
        char run[] = "run";
        require(command(run) == 0, "ngspice run failed");
        require(!state.callbackFault && !state.samples.empty(), "ngspice callback failed");
        final = state.samples.back();
        require(std::abs(final.time - target) <= 1e-12, "ngspice missed the debugger checkpoint");
    }
    char quit[] = "quit";
