"""Build only the owned freestanding firmware with an explicitly selected ARM GCC."""
...
def main() -> None:
    ...
    parser.add_argument('--toolchain', type=Path, required=True, help='Existing ARM GCC tools/bin directory; no download')
    parser.add_argument('--output', type=Path, default=ROOT / 'build/sn012/firmware')
    ...
    for line in symbols.splitlines():
        fields = line.split()
        if len(fields) == 3 and fields[2] in ('initialized_cookie', 'zero_cookie', '_stack_top', 'vectors', 'Reset_Handler'):
            selected[fields[2]] = int(fields[0], 16)
    if set(selected) != {'initialized_cookie', 'zero_cookie', '_stack_top', 'vectors', 'Reset_Handler'}:
    ...
    (output / 'firmware_layout.h').write_text(
        f'#define SN_DATA_ADDRESS {selected["initialized_cookie"]}u\n'
        f'#define SN_BSS_ADDRESS {selected["zero_cookie"]}u\n', encoding='utf-8')
    metadata = {'compiler_version': version, 'compiler_sha256': digest(compiler), 'flags': flags,
                ...
                'symbols': selected, 'sources_sha256': {p.name: digest(p) for p in
                    (HERE / 'firmware.c', HERE / 'firmware.ld', HERE / 'mailbox.h', HERE / 'build_firmware.py')}}
