"""Validate raw stopped-time ingress with the shared mailbox deadline using fixture pipes."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    names=("valid","wrong-token","legacy-token","unarmed","duplicate-arm","timeout","near-deadline",
           "commit-pending","other-command","arm-argument","early-arm","duplicate","normalized-bypass","disabled","normalized-time","wrong-time-token","missing-done","error-done","changed-time","bad-minute","duplicate-stream","actual-newline")
    try:
        for name in names:
