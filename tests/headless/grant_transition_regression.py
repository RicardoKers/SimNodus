"""Validate composite grant/open and observe/cancel transitions on actual Windows pipes."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    names = ("fixed", "paced", "partial-ready", "missing-ready", "late-ready", "backend-error",
             ...
             "duplicate-cancel", "invalid-duration", "unpaced", "negative-stop", "outside-stop")
    try:
        for name in names:
            ...
            try:
                ...
                requested_duration = 123 if name == "invalid-duration" else 100000000 if name == "unpaced" else duration
                ...
                started = command(f'grant-native {requested_duration} {int(name == "paced")} "{requested}"')
                ...
                if started["error"]:
                    final = started
                elif name == "duplicate-start": final = command(f'grant-native {requested_duration} {int(name == "paced")} "{requested}"')
                elif name == "observe-before-ready": final = command("observe 2010875")
                elif name == "cancel-before-ready": final = command("stop-native 2010875")
                    ...
                    if name == "broken-cancel":
                        os.close(read_fd); read_fd = None
                    timestamp = "-1" if name == "negative-stop" else "5000001" if name == "outside-stop" else "2010875"
                    final = command(f"stop-native {timestamp}")
                    if final["cancel_writes"]: wire("emulation CancelCancellationProbe\n")
                    if name == "duplicate-cancel": final = command("stop-native 2010875")
