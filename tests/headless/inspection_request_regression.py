"""Validate native register-pair correlation and shared deadline using fixture pipes."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    names=("valid","unarmed-inspection","before-readback","duplicate-arm","wrong-first","wrong-second","replay-first",
           "changed-second","missing-first","missing-second","late-second","shared-deadline","legacy-bypass","duplicate-result","extra-first")
    try:
        for name in names:
            ...
            prior=0
            try:
                command("enable-inspection-request")
                ...
                if name != "before-readback":
                    ...
                    if name=="shared-deadline":time.sleep(1.05)
                    if name=="unarmed-inspection":final=command("inspection-result "+json.dumps(first))
                    else:
                        armed=command("arm-inspection")
                        if armed["error"] or armed["inspection_request"]!={"token":3000003,"command":"3000003-data-list-register-values x 0 13 14"}:raise RuntimeError("Wrong first request")
                        if name=="duplicate-arm":final=command("arm-inspection")
                        elif name=="missing-first":final=command("commit")
                        elif name=="legacy-bypass":final=command(inspection)
                        else:
                            if name=="wrong-first":first=first.replace("3000003^","3000004^")
                            final=command("inspection-result "+json.dumps(first)+(" extra" if name=="extra-first" else ""))
                            if not final["error"]:
                                if final["inspection_verified"] or final["inspection_step"]!=2 or final["commits"]!=prior:raise RuntimeError("First read prematurely verified")
                                ...
                                if name=="missing-second":final=command("commit")
                                else:
                                    ...
                                    if name=="shared-deadline":time.sleep(.95)
                                    final=command("inspection-result "+json.dumps(second))
                                    if name=="duplicate-result":final=command("inspection-result "+json.dumps(second))
                if name == "valid":
