"""Validate native post-pair time confirmation and final commit ordering using fixture pipes."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    names=("valid","wrong-token","legacy-token","changed-time","bad-stream","missing-done","error-done",
           "duplicate","early-time","missing","extra","late-time","shared-budget","normalized","other-op")
    try:
        for name in names:
            ...
            def command(text):
                if text.startswith("readback "):
                    ...
                    memory=str(request["token"])+r'^done,memory=[{begin="0x20000000",offset="0x00000000",end="0x20000020",contents="'+content+'"}]'
                    stream=r'@"[Domain = Elapsed Virtual Time: 00:00:00.004027000\r\n"'
                    text="readback-timed "+" ".join(json.dumps(v) for v in (memory,stream,str(request["time_token"])+"^done"))
            ...
            prior=0
            try:
                command("enable-inspection-time")
                ...
                submit=lambda a,b:command("inspection-time-result "+json.dumps(a)+" "+json.dumps(b))
                if name=="early-time":final=submit(stream,done)
                else:
                    if name=="shared-budget":time.sleep(1.05)
                    if command("arm-inspection")["error"]:raise RuntimeError("Inspection setup failed")
                    first=registers.replace("93^","3000003^")
                    second=registers.replace("93^","4000003^")
                    if command("inspection-result "+json.dumps(first))["error"]:raise RuntimeError("First read failed")
                    while True:
                        released=command("release-inspection")
                        if released["error"]:raise RuntimeError("Release failed")
                        if released["inspection_step"]==2:break
                        time.sleep(max(.001,released["inspection_wait_ms"]/1000))
                    waiting=command("inspection-result "+json.dumps(second))
                    if waiting["error"] or waiting["inspection_step"]!=5 or waiting["inspection_verified"] or waiting["commits"]!=prior:
                        raise RuntimeError("Second read prematurely verified")
                    if waiting["inspection_time_request"]!={"token":5000003,"command":'5000003-interpreter-exec console "monitor machine ElapsedVirtualTime"'}:
                        raise RuntimeError("Wrong time request")
                    if name=="missing":final=command("commit")
                    elif name=="other-op":final=command("release-inspection")
                    else:
                        if name=="wrong-token":done="5000004^done"
                        if name=="legacy-token":done="2000003^done"
                        if name=="changed-time":stream=stream.replace("004027000","004027001")
                        if name=="bad-stream":stream=stream[:-1]
                        if name=="missing-done":done=""
                        if name=="error-done":done='5000003^error,msg="failed"'
                        if name=="late-time":time.sleep(2.01)
                        if name=="shared-budget":time.sleep(.85)
                        if name=="normalized":stream="4027000"
                        text="inspection-time-result "+json.dumps(stream)+" "+json.dumps(done)
                        if name=="extra":text+=" extra"
                        final=command(text)
                        if name=="duplicate":final=command(text)
                if name == "valid":
