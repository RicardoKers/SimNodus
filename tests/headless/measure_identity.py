"""Corrected identity-aware observer control; see OBSERVER_IDENTITY.md."""
import argparse
import json
import math
import os
...
from measure_baseline import ROOT, describe, sha, write
...
def main():
    ...
    prior = json.loads((ROOT / "build/sn018/observer-01/summary.json").read_text())
    preserved.update(prior["raw_files"])
    preserved[str(ROOT / "build/sn018/observer-01/summary.json")] = sha(ROOT / "build/sn018/observer-01/summary.json")
    verify(frozen)
    verify(preserved)
    for p in (Path(__file__), Path(__file__).with_name("OBSERVER_IDENTITY.md"), Path(__file__).with_name("process_memory.py"), Path(__file__).with_name("test_process_memory.py"), baseline_path):
        frozen[str(p)] = sha(p)
    modes = ["U", "S", "S", "U", "U", "S"]
        ...
        try:
            ...
            if mode == "S":
                ...
                batch["memory"] = dict(samples=len(samples), query_errors=sum(len(s["errors"]) for s in samples),
                    rejected_candidates=sum(len(s["rejected"]) for s in samples),
