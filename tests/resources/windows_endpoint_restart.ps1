# Disposable VMware endpoint and restart experiment.
...
public sealed class Sn021EndpointStore : IDisposable {
    ...
    public static void Seal(string leaf,string writer,string client,byte[] document,byte[] run,string path){
        using(var x=new Sn021EndpointStore()){SafeFileHandle store;x.Open(leaf,writer,out store);x.Stage="data";var data=x.Child(store,"data.bin",false);x.ExactAcl(data,writer,false);var content=x.Read(data,4096);
...
public static class Sn021EndpointPipe {
    ...
    public static byte[] Frame(byte[] document,byte[] run) {
        ...
        b[8]=1;b[10]=1;b[12]=32;Buffer.BlockCopy(Guid.NewGuid().ToByteArray(),0,b,16,16);
    ...
    public static string FrameError(byte[] b,int length,byte[] document,byte[] run) {
        ...
        if(b[8]!=1 || b[9]!=0)return "request-version";
    ...
    static void ClientClosed(SafeFileHandle h,Stopwatch clock) {
        try {uint n=Io(h,1,new byte[65],clock,5000);Require(n==0,"extra-request");}
    ...
    public static Observation RunQuery(string role,string pipe,string writer,string client,string other,string ready,string leaf,string test,byte[] document,byte[] run,byte[] expectedDigest,byte[] replay) {
        ...
        try {
            ...
            if(role=="server") {
                ...
                using(var store=new Sn021EndpointStore()) {
                    ...
                    using(var h=Create(pipe,writer,client,other)) {
                        ...
                        o.Stage="reply";Require(Io(h,2,reply,clock,5000)==64,"short-reply-write");o.ResponseBytes=64;o.Authorized=true;
                        o.Stage="connection-end";ClientClosed(h,clock);o.Status="query-served";
                    }
                ...
                using(var h=CreateFileW(pipe,ClientRights,0,IntPtr.Zero,3,test=="anonymous"?0x40100000u:0x40110000u,IntPtr.Zero)) {
                    ...
                    byte[] request=replay.Length==0?Request(test,document,run):replay;Require(request.Length==64,"replay-size");o.RequestHex=BitConverter.ToString(request).Replace("-","").ToLowerInvariant();o.RequestBytes=(int)Io(h,2,request,clock,5000);
                    ...
                    Require(Match(reply,0,System.Text.Encoding.ASCII.GetBytes("SN21IPC1")) && reply[8]==1 && reply[9]==0 && reply[10]==1 && reply[11]==128 && reply[12]==32 && reply[13]==0 && reply[14]==0 && reply[15]==0,"response-header");
                    ...
                    // Closing after receipt lets the server finish without an extra protocol message.
                }
            }
    ...
    // Controlled hostile listener: records receipt, never replies or accesses a store.
    public static Observation Listen(string pipe,string owner,string client,string ready,bool extraRights) {
        var o=new Observation();var clock=Stopwatch.StartNew();o.ProcessSid=Sid();LastCancellation=null;
        try {
            Require(o.ProcessSid==owner,"wrong-listener-token");o.Stage="create-pipe";
            using(var h=Create(pipe,owner,client,"",extraRights)) {
                File.WriteAllText(ready,"ready");o.Stage="connect";Io(h,0,new byte[1],clock,15000);
                clock.Restart();o.Stage="read";var data=new byte[65];o.RequestBytes=(int)Io(h,1,data,clock,5000);
                o.Status="unexpected-receipt";
            }
        } catch(Exception e) {o.Status="failed";o.ErrorType=e.GetType().FullName;var w=e as Win32Exception;if(w!=null)o.NativeError=w.NativeErrorCode;if(e is InvalidOperationException || e is TimeoutException)o.Failure=e.Message;}
        finally{o.ElapsedMs=clock.ElapsedMilliseconds;o.Cancellation=LastCancellation;}
        return o;
    }
...
'@
    if($Mode -in @('Server','Client','Listener')){
        ...
        $pipe="\\.\pipe\SN021Endpoint-$Tag-$Case";$ready=Join-Path $harness "writer\$Instance.ready"
        if($Mode -eq 'Listener'){
            $owner=if($Case -eq 'squatter'){$OtherSid}else{$WriterSid}
            if($Case -eq 'squatter'){$ready=Join-Path $harness "unauthorized\$Instance.ready"}
            $report.result=[Sn021EndpointPipe]::Listen($pipe,$owner,$ClientSid,$ready,($Case -eq 'rights'))
        }else{
            [byte[]]$replay=@();if($RequestHex -ne 'none'){$replay=Bytes $RequestHex}
            $report.result=[Sn021EndpointPipe]::RunQuery($Mode.ToLowerInvariant(),$pipe,$WriterSid,$ClientSid,'',$ready,"SN021Endpoint-$Tag-$Case",'good',(Bytes $DocumentHex),(Bytes $RunHex),(Bytes $DigestHex),$replay)
        }
    ...
    foreach($role in @('w','c','u')){
        $name='sne'+$role+$Tag.Substring(0,10);Require (-not (Get-LocalUser -Name $name -ErrorAction SilentlyContinue)) 'account-exists'
    ...
    function Hashes {return [ordered]@{data=(Get-FileHash $data -Algorithm SHA256).Hash;manifest=(Get-FileHash $manifest -Algorithm SHA256).Hash}}
    function Record([string]$Name,$Before,$Server,$Client,$Contender){
        $after=Hashes;$preserved=$Before.data -eq $after.data -and $Before.manifest -eq $after.manifest
        $report.observations+=[ordered]@{case=$Name;before=$Before;after=$after;bytes_preserved=$preserved;server=$Server;client=$Client;contender=$Contender;document=$DocumentHex;run=$RunHex;endpoint="SN021Endpoint-$Tag-$Case"};Save-Report
        Require $preserved 'fixture-changed'
    }
    foreach($Case in @('squatter','rights','competitor','restart')){
        $report.stage=$Case;Save-Report
        $leaf="SN021Endpoint-$Tag-$Case";$root="C:\$leaf";$store=Join-Path $root 'store'
        Require (-not (Test-Path $root)) 'fixture-exists';Protect $root 'S-1-5-32-545';Protect $store $WriterSid
        $data=Join-Path $store 'data.bin';$manifest=Join-Path $store 'manifest.bin';[IO.File]::WriteAllText($data,'endpoint fixture',$utf8)
        ...
        [Sn021EndpointStore]::Seal($leaf,$WriterSid,$ClientSid,(Bytes $DocumentHex),(Bytes $RunHex),$manifest)
        $before=Hashes;$RequestHex='none';$contender=$null
        if($Case -in @('squatter','rights')){
            $role=if($Case -eq 'squatter'){'u'}else{'w'}
            $server=Start-Child $role 'Listener' ($Case+'listener');Await-Ready $server
        }else{$server=Start-Child 'w' 'Server' ($Case+'first');Await-Ready $server}
        if($Case -in @('squatter','competitor')){
            $second=Start-Child 'w' 'Server' ($Case+'second');$contender=Finish $second
            $originalAlive=-not $server.process.HasExited
            $report.observations+=[ordered]@{case=($Case+'occupation');contender=$contender;endpoint_ready=(Test-Path $second.ready);original_server_alive=$originalAlive};Save-Report
            # The measured same-SID refusal is ERROR_PIPE_BUSY with one instance.
            # Accept only busy/access-denied at creation while the original is alive.
            $expectedError=$contender.result.NativeError -eq 5 -or ($Case -eq 'competitor' -and $contender.result.NativeError -eq 231)
            Require ($originalAlive -and $contender.result.Stage -eq 'create-pipe' -and $expectedError -and -not (Test-Path $second.ready)) 'occupied-name-not-refused'
        }
        $client=Start-Child 'c' 'Client' ($Case+'client');$cl=Finish $client;$sv=Finish $server
        Record $Case $before $sv $cl $contender
        if($Case -in @('squatter','rights')){
            $failure=if($Case -eq 'squatter'){'wrong-pipe-owner'}else{'wrong-pipe-rights'}
            Require ($cl.result.Stage -eq 'verify-server' -and $cl.result.Failure -eq $failure -and $cl.result.RequestBytes -eq 0 -and -not $cl.result.ReplyReceived) 'endpoint-rejection-inconclusive'
            Require ($sv.result.Stage -eq 'read' -and $sv.result.NativeError -in @(109,232) -and $sv.result.RequestBytes -eq 0) 'listener-received-or-inconclusive'
        }else{
            Require ($sv.result.Status -eq 'query-served' -and $cl.result.Status -eq 'query-received' -and $sv.result.HandlesRetained -and $sv.result.Reverted) 'good-query-failed'
        }
        if($Case -ne 'restart'){continue}
        $oldRun=$RunHex;$oldRequest=$cl.result.RequestHex;$oldPid=$sv.result.Pid;$rootId=$sv.result.RootIdentity;$fileId=$sv.result.FileIdentity
        Require ($oldRequest -eq $sv.result.RequestHex -and $oldRequest.Length -eq 128) 'request-capture-mismatch'
        # All prior processes have exited. Setup rotates only the protected run slot.
        $RunHex=[guid]::NewGuid().ToString('N');$manifestBefore=[IO.File]::ReadAllBytes($manifest)
        $manifestAfter=[byte[]]$manifestBefore.Clone();[Array]::Copy((Bytes $RunHex),0,$manifestAfter,80,16)
        [IO.File]::WriteAllBytes($manifest,$manifestAfter)
        $actual=[IO.File]::ReadAllBytes($manifest);$onlyRun=$actual.Length -eq 224
        for($i=0;$i -lt $actual.Length;$i++){if(($i -lt 80 -or $i -ge 96) -and $actual[$i] -ne $manifestBefore[$i]){$onlyRun=$false}}
        $transitionAfter=Hashes
        $report.transition=[ordered]@{old_run=$oldRun;new_run=$RunHex;previous_process_exited=$server.process.HasExited;old_pid=$oldPid;request_hex=$oldRequest;before=$before;after=$transitionAfter;only_run_slot_changed=$onlyRun};Save-Report
        Require ($onlyRun -and $oldRun -ne $RunHex -and $before.data -eq $transitionAfter.data -and $before.manifest -ne $transitionAfter.manifest) 'rotation-inconclusive'
        foreach($name in @('stale','fresh','corrupt')){
            $RequestHex=if($name -eq 'stale'){$oldRequest}else{'none'}
            if($name -eq 'corrupt'){[IO.File]::WriteAllText($data,'modified fixture',$utf8)}
            $before=Hashes;$server=Start-Child 'w' 'Server' $name
            if($name -eq 'corrupt'){
                $sv=Finish $server;$report.corrupt_endpoint_ready=Test-Path $server.ready;Record $name $before $sv $null $null
                Require (-not (Test-Path $server.ready) -and $sv.result.Stage -eq 'startup' -and $sv.result.Failure -eq 'file-bytes') 'restart-corruption-not-refused'
                continue
            }
            Await-Ready $server;$client=Start-Child 'c' 'Client' ($name+'client');$cl=Finish $client;$sv=Finish $server
            Record $name $before $sv $cl $null
            Require ($sv.result.HandlesRetained -and $sv.result.RootIdentity -eq $rootId -and $sv.result.FileIdentity -eq $fileId -and $sv.result.Pid -ne $oldPid) 'restart-identity-inconclusive'
            if($name -eq 'stale'){
                Require ($cl.result.RequestHex -eq $oldRequest -and $sv.result.RequestHex -eq $oldRequest -and $sv.result.Stage -eq 'frame' -and $sv.result.Failure -eq 'request-run' -and $sv.result.Reverted -and -not $cl.result.ReplyReceived) 'stale-replay-not-refused'
            }else{Require ($sv.result.Status -eq 'query-served' -and $cl.result.Status -eq 'query-received') 'fresh-restart-query-failed'}
        }
    }
    $report.elapsed_ms=$probeClock.ElapsedMilliseconds
    Require ($report.elapsed_ms -lt 180000) 'run-deadline-left-for-inspection'
    $report.status='observed-endpoint-restart-slice-only';$report.stage='complete'
    $report.limits=@('Administrator-controlled run rotation, not production lifecycle','Orderly process exit, not crash recovery','No forged response/duplicate/dribbling matrix','Writer identity and administrator remain trusted','No save/version/ABA or service')
}catch{
