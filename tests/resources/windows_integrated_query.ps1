# Disposable VMware integrated read-only query experiment.
param([ValidateSet('Inventory','Probe','Server','Client')][string]$Mode,
...
function Launch($Credential,[string]$ChildMode,[string]$Destination,[string]$Other){
    $args=@('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',('"'+(Join-Path $harness 'probe.ps1')+'"'),'-Mode',$ChildMode,'-Tag',$Tag,'-OutputPath',('"'+$Destination+'"'),'-WriterSid',$WriterSid,'-ClientSid',$ClientSid,'-OtherSid',$Other,'-Case',$Case,'-DocumentHex',$DocumentHex,'-RunHex',$RunHex,'-DigestHex',$DigestHex)
...
public sealed class Sn021QueryStore : IDisposable {
    ...
    public static void Seal(string leaf,string writer,string client,byte[] document,byte[] run,string path){
        using(var x=new Sn021QueryStore()){SafeFileHandle store;x.Open(leaf,writer,out store);x.Stage="data";var data=x.Child(store,"data.bin",false);x.ExactAcl(data,writer,false);var content=x.Read(data,4096);
...
public static class Sn021QueryPipe {
    ...
    public sealed class Observation {
        public string Failure,Cancellation,RootIdentity,FileIdentity;
    ...
    public static Observation RunQuery(string role,string pipe,string writer,string client,string other,string ready,string leaf,string test,byte[] document,byte[] run,byte[] expectedDigest) {
        ...
        try {
            ...
            if(role=="server") {
                ...
                using(var store=new Sn021QueryStore()) {
                    ...
                    using(var h=Create(pipe,writer,client,other)) {
                        ...
                        clock.Restart();o.Stage="read";var request=new byte[65];uint n=Io(h,1,request,clock,5000);o.RequestBytes=(int)n;
                ...
                using(var h=CreateFileW(pipe,ClientRights,0,IntPtr.Zero,3,test=="anonymous"?0x40100000u:0x40110000u,IntPtr.Zero)) {
                    ...
                    byte[] request=Request(test,document,run);Io(h,2,request,clock,5000);
...
'@
    if($Mode -in @('Server','Client')){
        ...
        $report.result=[Sn021QueryPipe]::RunQuery($Mode.ToLowerInvariant(),"\\.\pipe\SN021Query-$Tag-$Case",$WriterSid,$ClientSid,$other,(Join-Path $harness "writer\$Case.ready"),"SN021Query-$Tag-$Case",$Case,(Bytes $DocumentHex),(Bytes $RunHex),(Bytes $DigestHex))
    ...
    foreach($role in @('w','c','u')){
        $name='snq'+$role+$Tag.Substring(0,10);Require (-not (Get-LocalUser -Name $name -ErrorAction SilentlyContinue)) 'account-exists'
        $password=ConvertTo-SecureString ('Sn!9'+[guid]::NewGuid().ToString('N')) -AsPlainText -Force
        New-LocalUser -Name $name -Password $password -Description 'Disposable SN-021 integrated query fixture'|Out-Null
    ...
    $WriterSid=$sids.w;$ClientSid=$sids.c;$report.principals=$sids
    Protect $harness 'S-1-5-32-545';Protect (Join-Path $harness 'writer') 'S-1-5-32-545' $sids.w
    Protect (Join-Path $harness 'client') '' $sids.c;Protect (Join-Path $harness 'unauthorized') '' $sids.u
    Copy-Item -LiteralPath $PSCommandPath -Destination (Join-Path $harness 'probe.ps1')
    $frameErrors=@{magic='request-magic';version='request-version';length='request-body-length';save='request-operation';document='request-document';run='request-run';path='request-document';short='request-size'}
    foreach($Case in @('goodone','goodtwo','handlerdeny','anonymous','pipedeny','magic','version','length','save','document','run','path','short','oversized','extra','slow','disconnect','startupbytes')){
        $report.stage=$Case;Save-Report
        $leaf="SN021Query-$Tag-$Case";$root="C:\$leaf";$store=Join-Path $root 'store'
        ...
        [Sn021QueryStore]::Seal($leaf,$WriterSid,$ClientSid,(Bytes $DocumentHex),(Bytes $RunHex),$manifest)
        if($Case -eq 'startupbytes'){[IO.File]::WriteAllText($data,'other fixture',$utf8)}
        $before=[ordered]@{data_sha256=(Get-FileHash $data -Algorithm SHA256).Hash;manifest_sha256=(Get-FileHash $manifest -Algorithm SHA256).Hash;document=$DocumentHex;run=$RunHex;expected_response_digest=$DigestHex}
        $other=if($Case -eq 'handlerdeny'){$sids.u}else{'none'};$role=if($Case -in @('handlerdeny','pipedeny')){'u'}else{'c'}
        $serverOutput=Join-Path $harness "writer\$Case.json";$directory=if($role -eq 'u'){'unauthorized'}else{'client'};$clientOutput=Join-Path $harness "$directory\$Case.json"
        $ready=Join-Path $harness "writer\$Case.ready";$server=Launch $credentials.w 'Server' $serverOutput $other
        $wait=[Diagnostics.Stopwatch]::StartNew()
        while(-not (Test-Path $ready) -and -not $server.HasExited -and $wait.ElapsedMilliseconds -lt 20000){Start-Sleep -Milliseconds 100}
        if($Case -eq 'startupbytes'){
            Require ($server.WaitForExit(30000) -and (Test-Path $serverOutput)) 'missing-startup-report'
            $sv=Get-Content $serverOutput -Raw|ConvertFrom-Json
            $unchanged=(Get-FileHash $data -Algorithm SHA256).Hash -eq $before.data_sha256 -and (Get-FileHash $manifest -Algorithm SHA256).Hash -eq $before.manifest_sha256
            $report.observations+=[ordered]@{case=$Case;before=$before;server=$sv;endpoint_ready=(Test-Path $ready);bytes_preserved=$unchanged};Save-Report
            Require ($unchanged -and -not (Test-Path $ready) -and $sv.status -eq 'observed' -and $sv.token.sid -eq $WriterSid -and $sv.result.Stage -eq 'startup' -and $sv.result.Failure -eq 'file-bytes') 'startup-not-refused'
            continue
        }
        if(-not (Test-Path $ready)){if(Test-Path $serverOutput){$report.observations+=[ordered]@{case=$Case;server=(Get-Content $serverOutput -Raw|ConvertFrom-Json)};Save-Report};throw 'PROBE:server-not-ready'}
        $client=Launch $credentials[$role] 'Client' $clientOutput $other
        Require ($client.WaitForExit(30000)) 'client-timeout-left-for-inspection';Require ($server.WaitForExit(30000)) 'server-timeout-left-for-inspection'
        Require ((Test-Path $serverOutput) -and (Test-Path $clientOutput)) 'missing-child-report'
        $sv=Get-Content $serverOutput -Raw|ConvertFrom-Json;$cl=Get-Content $clientOutput -Raw|ConvertFrom-Json
        $unchanged=(Get-FileHash $data -Algorithm SHA256).Hash -eq $before.data_sha256 -and (Get-FileHash $manifest -Algorithm SHA256).Hash -eq $before.manifest_sha256
        $report.observations+=[ordered]@{case=$Case;before=$before;server=$sv;client=$cl;bytes_preserved=$unchanged};Save-Report
        Require ($unchanged -and $sv.status -eq 'observed' -and $cl.status -eq 'observed') 'child-harness-or-preservation-failed'
        Require ($sv.token.sid -eq $WriterSid -and $cl.token.sid -eq $sids[$role]) 'wrong-child-sid'
        if($Case -in @('goodone','goodtwo')){Require ($sv.result.Status -eq 'query-served' -and $cl.result.Status -eq 'query-received' -and $sv.result.Reverted -and $sv.result.HandlesRetained -and $sv.result.ResponseBytes -eq 64) 'good-query-failed'}
        elseif($frameErrors.ContainsKey($Case)){Require ($sv.result.Stage -eq 'frame' -and $sv.result.Failure -eq $frameErrors[$Case] -and -not $cl.result.ReplyReceived) 'frame-refusal-inconclusive'}
        elseif($Case -eq 'handlerdeny'){Require ($sv.result.Stage -eq 'authorize' -and $sv.result.Failure -eq 'unauthorized-document-principal' -and $sv.result.AuthenticatedSid -eq $sids.u -and $sv.result.Reverted -and -not $cl.result.ReplyReceived) 'authorization-refusal-inconclusive'}
        elseif($Case -eq 'anonymous'){Require ($sv.result.Stage -eq 'identify' -and $sv.result.NativeError -eq 1347 -and $sv.result.Reverted -and -not $cl.result.ReplyReceived) 'anonymous-refusal-inconclusive'}
        elseif($Case -eq 'pipedeny'){Require ($cl.result.Stage -eq 'open-pipe' -and $cl.result.NativeError -eq 5 -and $sv.result.Stage -eq 'connect' -and $sv.result.ErrorType -eq 'System.TimeoutException' -and $sv.result.Cancellation) 'pipe-refusal-inconclusive'}
        elseif($Case -eq 'oversized'){Require ($sv.result.Stage -eq 'read' -and $sv.result.NativeError -eq 234 -and -not $cl.result.ReplyReceived) 'oversize-refusal-inconclusive'}
        elseif($Case -eq 'extra'){Require ($sv.result.Stage -eq 'connection-end' -and $sv.result.Failure -eq 'extra-request') 'extra-request-inconclusive'}
        elseif($Case -eq 'slow'){Require ($sv.result.Stage -eq 'read' -and $sv.result.ErrorType -eq 'System.TimeoutException' -and $sv.result.Cancellation -and -not $cl.result.ReplyReceived) 'deadline-inconclusive'}
        elseif($Case -eq 'disconnect'){Require ($sv.result.Stage -eq 'read' -and $sv.result.NativeError -in @(109,232) -and -not $sv.result.Authorized) 'disconnect-inconclusive'}
    }
    $report.status='observed-integrated-query-slice-only';$report.stage='complete'
    $report.limits=@('No hostile endpoint/squatter case yet','No complete restart/stale-run lifecycle','One request per server process','Extra-message refusal can follow the first valid reply','No save/version/ABA or recovery','No service or production API')
}catch{
