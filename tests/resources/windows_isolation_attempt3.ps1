    ...
    if($Mode -in @('Server','Client','Attack')){
        ...
        if($Mode -eq 'Attack'){
            ...
            $operations=[ordered]@{
                ...
                rename={ [IO.File]::Move($target,(Join-Path $store 'client-renamed.bin')) }
                ...
                hardlink={ New-Item -ItemType HardLink -Path (Join-Path (Split-Path $OutputPath) ($Instance+'-link.bin')) -Target $target|Out-Null }
