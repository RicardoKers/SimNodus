...
param([ValidateSet('Inventory','Probe','Server','Client','Attack')][string]$Mode,
...
// Direct Win32 calls preserve native failures instead of managed existence checks.
public static class Sn021IsolationMutations {
    ...
    [System.Runtime.InteropServices.DllImport("kernel32.dll",CharSet=System.Runtime.InteropServices.CharSet.Unicode,SetLastError=true)]
    static extern bool ReplaceFileW(string existing,string replacement,string backup,uint flags,System.IntPtr exclude,System.IntPtr reserved);
    [System.Runtime.InteropServices.DllImport("kernel32.dll",CharSet=System.Runtime.InteropServices.CharSet.Unicode,SetLastError=true)]
    static extern bool CreateHardLinkW(string alias,string existing,System.IntPtr security);
    static void Check(bool value){if(!value)throw new System.ComponentModel.Win32Exception(System.Runtime.InteropServices.Marshal.GetLastWin32Error());}
    public static void Rename(string existing,string destination){Check(MoveFileExW(existing,destination,0));}
    public static void Delete(string path){Check(DeleteFileW(path));}
    public static void Replace(string existing,string replacement){Check(ReplaceFileW(existing,replacement,null,0,System.IntPtr.Zero,System.IntPtr.Zero));}
    public static void Link(string alias,string existing){Check(CreateHardLinkW(alias,existing,System.IntPtr.Zero));}
}

'@
    if($Mode -in @('Server','Client','Attack')){
        ...
        $pipe="\\.\pipe\SN021Isolation-$Tag-$Case";$ready=Join-Path $harness "writer\$Instance.ready"
        if($Mode -eq 'Attack'){
            Require ($report.token.sid -eq $ClientSid) 'wrong-attack-principal'
            $root="C:\SN021Isolation-$Tag-$Case";$store=Join-Path $root 'store';$target=Join-Path $store 'data.bin'
            $candidate=Join-Path (Split-Path $OutputPath) ($Instance+'-candidate.bin');[IO.File]::WriteAllText($candidate,'client replacement',$utf8)
            $operations=[ordered]@{
                write={ [IO.File]::WriteAllText($target,'client write',$utf8) }
                manifest_write={ [IO.File]::WriteAllText((Join-Path $store 'manifest.bin'),'client metadata',$utf8) }
                create={ [IO.File]::WriteAllText((Join-Path $store 'client-created.bin'),'client create',$utf8) }
                rename={ [Sn021IsolationMutations]::Rename($target,(Join-Path $store 'client-renamed.bin')) }
                delete={ [Sn021IsolationMutations]::Delete($target) }
                replace={ [Sn021IsolationMutations]::Replace($target,$candidate) }
                hardlink={ [Sn021IsolationMutations]::Link((Join-Path (Split-Path $OutputPath) ($Instance+'-link.bin')),$target) }
                change_acl={ $acl=Get-Acl -LiteralPath $target;$acl.AddAccessRule((New-Object Security.AccessControl.FileSystemAccessRule(([Security.Principal.SecurityIdentifier]$ClientSid),'FullControl','Allow')));Set-Acl -LiteralPath $target -AclObject $acl }
                change_owner={ $acl=Get-Acl -LiteralPath $target;$acl.SetOwner([Security.Principal.SecurityIdentifier]$ClientSid);Set-Acl -LiteralPath $target -AclObject $acl }
                rename_store={ [Sn021IsolationMutations]::Rename($store,(Join-Path $root 'client-store')) }
            }
            foreach($name in $operations.Keys){
                $o=[ordered]@{operation=$name;denied=$false}
                try{& $operations[$name]}catch{
                    $ex=$_.Exception;while($ex.InnerException){$ex=$ex.InnerException}
                    $o.error_type=$ex.GetType().FullName;$o.error_code=$ex.HResult -band 65535
                    if($ex -is [ComponentModel.Win32Exception]){$o.error_code=$ex.NativeErrorCode}
                    $o.denied=$o.error_code -in @(5,1314)
                }
                $report.observations+=$o;Save-Report
                Require $o.denied ('unexpected-attack-outcome-'+$name)
            }

        }else{
            [byte[]]$replay=@();if($RequestHex -ne 'none'){$replay=Bytes $RequestHex}
            $other=if($Case -eq 'handlerdeny'){$OtherSid}else{''}
            $report.status='validated-child-token';Save-Report
            $report.result=[Sn021MatrixPipe]::RunQuery($Mode.ToLowerInvariant(),$pipe,$WriterSid,$ClientSid,$other,$ready,"SN021Isolation-$Tag-$Case",$Case,(Bytes $DocumentHex),(Bytes $RunHex),(Bytes $DigestHex),$replay)
        }
    ...
    foreach($role in @('w','c','u')){
        $name='sni'+$role+$Tag.Substring(0,10);Require (-not (Get-LocalUser -Name $name -ErrorAction SilentlyContinue)) 'account-exists'
    ...
    function Finish($Child){
        ...
        if($v.status -ne 'observed' -or $v.token.sid -ne $sids[$Child.role]){$report.observations+=[ordered]@{case=$Child.label;partial=$v};Save-Report}
        Require ($v.status -eq 'observed' -and $v.token.sid -eq $sids[$Child.role]) 'child-harness-or-sid'
        return $v
    }
    function Snapshot {
        $s=[ordered]@{}
        foreach($path in @($root,$store,$data,$manifest)){
            $entry=[ordered]@{identity=[Sn021FileIdentity]::Read($path);security=(Get-Acl -LiteralPath $path).Sddl}
            if([IO.File]::Exists($path)){$entry.hash=(Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash}
            $s[$path]=$entry
        }
        return ($s|ConvertTo-Json -Depth 6 -Compress)
    }
    function Attack-Phase([string]$Label,$Writer){
        $before=Snapshot;$liveBefore=if($Writer){-not $Writer.process.HasExited}else{$false}
        if($Writer){Require $liveBefore 'writer-not-alive-before-attack'}
        $attacker=Start-Child 'c' 'Attack' $Label;$attack=Finish $attacker
        $after=Snapshot;$liveAfter=if($Writer){-not $Writer.process.HasExited}else{$false}
        $report.observations+=[ordered]@{case=$Label;before=$before;after=$after;preserved=($before -eq $after);writer_alive_before=$liveBefore;writer_alive_after=$liveAfter;attacker=$attack};Save-Report
        Require ($before -eq $after -and $attack.observations.Count -eq 10) 'attack-preservation-inconclusive'
        if($Writer){Require $liveAfter 'writer-exited-during-attack'}
    }
    function Query([string]$Label,$Server){
        $before=Snapshot;$role=if($Case -in @('handlerdeny','pipedeny')){'u'}else{'c'}
        $client=Start-Child $role 'Client' ($Label+'client');$cl=Finish $client;$sv=Finish $Server
        $after=Snapshot;$record=[ordered]@{case=$Label;before=$before;after=$after;preserved=($before -eq $after);server=$sv;client=$cl;endpoint="SN021Isolation-$Tag-$Case";document=$DocumentHex;run=$RunHex}
        $report.observations+=$record;Save-Report;Require ($before -eq $after) 'query-changed-fixture'
        return $record
    }
    $errors=@{document='request-document';run='request-run';path='request-document';save='request-operation'}
    foreach($Case in @('goodone','goodtwo','handlerdeny','anonymous','pipedeny','document','run','path','save','lifetime')){
        $report.stage=$Case;Save-Report
        $leaf="SN021Isolation-$Tag-$Case";$root="C:\$leaf";$store=Join-Path $root 'store'
        ...
        $RequestHex='none';$server=Start-Child 'w' 'Server' ($Case+'server');Await-Ready $server
        $q=Query $Case $server;$s=$q.server.result;$c=$q.client.result
        if($Case -in @('goodone','goodtwo','lifetime')){
            Require ($s.Status -eq 'query-served' -and $s.Reverted -and $s.HandlesRetained -and $c.Status -eq 'query-received') 'good-query-failed'
        }elseif($Case -eq 'handlerdeny'){
            Require ($s.Stage -eq 'authorize' -and $s.Failure -eq 'unauthorized-document-principal' -and $s.AuthenticatedSid -eq $sids.u -and $s.Reverted -and -not $c.ReplyReceived) 'handler-refusal-inconclusive'
        }elseif($Case -eq 'anonymous'){
            Require ($s.Stage -eq 'identify' -and $s.NativeError -eq 1347 -and $s.Reverted -and -not $c.ReplyReceived) 'anonymous-refusal-inconclusive'
        }elseif($Case -eq 'pipedeny'){
            Require ($c.Stage -eq 'open-pipe' -and $c.NativeError -eq 5 -and $s.Stage -eq 'connect' -and $s.ErrorType -eq 'System.TimeoutException' -and $s.Cancellation) 'pipe-refusal-inconclusive'
        }else{Require ($s.Stage -eq 'frame' -and $s.Failure -eq $errors[$Case] -and $s.Reverted -and -not $c.ReplyReceived) 'frame-refusal-inconclusive'}
        if($Case -ne 'lifetime'){continue}
        $oldRequest=$c.RequestHex;$oldRun=$RunHex;$rootId=$s.RootIdentity;$fileId=$s.FileIdentity
        Require ($s.RequestHex -eq $oldRequest -and $oldRequest.Length -eq 128) 'capture-mismatch'
        $server=Start-Child 'w' 'Server' 'victim';Await-Ready $server
        Attack-Phase 'liveattack' $server
        $beforeKill=Snapshot;$tokenReport=Get-Content $server.output -Raw|ConvertFrom-Json
        Require ($tokenReport.status -eq 'validated-child-token' -and $tokenReport.token.sid -eq $WriterSid -and -not $server.process.HasExited) 'victim-identity-inconclusive'
        # Kill only the retained Process object launched above in this disposable fixture.
        $server.process.Kill();Require ($server.process.WaitForExit(10000)) 'victim-kill-inconclusive'
        $report.termination=[ordered]@{pid=$server.process.Id;exited=$server.process.HasExited;token=$tokenReport.token;before=$beforeKill;after=(Snapshot);no_final_server_result=$true};Save-Report
        Require ($report.termination.before -eq $report.termination.after) 'termination-changed-fixture'
        Attack-Phase 'stoppedattack' $null
        $beforeRotation=Snapshot;$bytes=[IO.File]::ReadAllBytes($manifest);$original=[byte[]]$bytes.Clone();$RunHex=[guid]::NewGuid().ToString('N');[Array]::Copy((Bytes $RunHex),0,$bytes,80,16);[IO.File]::WriteAllBytes($manifest,$bytes)
        $actual=[IO.File]::ReadAllBytes($manifest);$onlyRun=$actual.Length -eq 224
        for($i=0;$i -lt $actual.Length;$i++){if(($i -lt 80 -or $i -ge 96) -and $actual[$i] -ne $original[$i]){$onlyRun=$false}}
        $report.rotation=[ordered]@{old_run=$oldRun;new_run=$RunHex;only_run_slot_changed=$onlyRun;before=$beforeRotation;after=(Snapshot);old_request=$oldRequest};Save-Report
        Require ($onlyRun -and $oldRun -ne $RunHex) 'rotation-inconclusive'
        $RequestHex=$oldRequest;$server=Start-Child 'w' 'Server' 'stale';Await-Ready $server
        Attack-Phase 'restartedattack' $server
        $stale=Query 'stale' $server;$ss=$stale.server.result
        Require ($ss.Stage -eq 'frame' -and $ss.Failure -eq 'request-run' -and $ss.RequestHex -eq $oldRequest -and $stale.client.result.RequestHex -eq $oldRequest -and -not $stale.client.result.ReplyReceived -and $ss.Reverted) 'stale-replay-inconclusive'
        $RequestHex='none';$server=Start-Child 'w' 'Server' 'fresh';Await-Ready $server;$fresh=Query 'fresh' $server
        foreach($result in @($ss,$fresh.server.result)){Require ($result.RootIdentity -eq $rootId -and $result.FileIdentity -eq $fileId -and $result.HandlesRetained) 'restart-store-identity'}
        Require ($fresh.server.result.Status -eq 'query-served' -and $fresh.client.result.Status -eq 'query-received') 'fresh-query-failed'
    }
    $report.elapsed_ms=$probeClock.ElapsedMilliseconds;Require ($report.elapsed_ms -lt 180000) 'run-deadline-left-for-inspection'
    $report.status='observed-isolation-prerequisites-only';$report.stage='complete'
    $report.limits=@('Forced termination occurs during read-only connection wait, not commit recovery','Run rotation remains administrator-controlled','Ten listed mutation sequences only; ACL/owner sequence can reject at its initial read','No service or production save implementation','Unchanged private-v2 protocol; no OS-reboot or power-loss claim')
}catch{
    ...
    $ex=$_.Exception;while($ex.InnerException){$ex=$ex.InnerException};if($ex -is [ComponentModel.Win32Exception]){$report.native_error=$ex.NativeErrorCode};if($ex -is [InvalidOperationException]){$report.detail=$ex.Message}
}finally{Save-Report}
