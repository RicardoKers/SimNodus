...
'@
    ...
    foreach($Case in @('squatter','rights','competitor','restart')){
        ...
        if($Case -in @('squatter','competitor')){
            ...
            $report.observations+=[ordered]@{case=($Case+'occupation');contender=$contender;endpoint_ready=(Test-Path $second.ready)};Save-Report
            Require ($contender.result.Stage -eq 'create-pipe' -and $contender.result.NativeError -eq 5 -and -not (Test-Path $second.ready)) 'occupied-name-not-refused'
        }
