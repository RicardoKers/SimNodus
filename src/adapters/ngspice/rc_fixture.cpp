...
{
    require(argc == 7, "Usage: simnodus_rc_fixture <dll> <audio-directory> <initialization> <target-ns> <rise-ns-or-none> <fall-ns-or-none>");
    ...
    while(std::getline(std::cin, request))
    {
        ...
        require(simnodus::RcFixtureContract::request(currentNs, ns, targetNanoseconds)
                == simnodus::BoundaryError::none, "Invalid persistent advance boundary");
        ...
        if(simnodus::RcFixtureContract::observation({ns, final.time, final.output})
                != simnodus::BoundaryError::none)
        {
...
int simnodus::ngspice::run_rc_fixture(int argc, wchar_t** argv)
{
    try
    {
        execute(argc, argv);
    ...
    {
        std::cerr << "RC fixture failed: " << error.what() << '\n';
        return 1;
    }
}
